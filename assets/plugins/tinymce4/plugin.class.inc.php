...
class TinyMCE4
{
    ...
    function get_mce_script()
    {
        ...
        $cfg['height']      = !empty( $params['height'] ) ? $params['height'] : '400px';
        $cfg['width']       = !empty( $params['width'] ) ? $params['width'] : '100%';
        ...
        $cfg['plugins'] =  isset( $params['plugins'] ) ? $params['plugins'] : "advlist autolink lists link image charmap print preview hr anchor pagebreak searchreplace wordcount visualblocks visualchars code fullscreen spellchecker insertdatetime media nonbreaking save table contextmenu directionality emoticons template paste textcolor codesample colorpicker textpattern imagetools paste modxlink";
        $cfg['paste_word_valid_elements'] = !empty( $params['paste_word_valid_elements'] ) ? $params['paste_word_valid_elements'] : 'b,strong,i,em,h1,h2,table';
        //template forecolor backcolor
        $cfg['toolbar1'] = !empty( $params['toolbar1'] ) ? $params['toolbar1'] : "undo redo | cut copy paste | searchreplace | bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent blockquote | styleselect";
        $cfg['toolbar2'] =  isset( $params['toolbar2'] ) ? $params['toolbar2'] : "link unlink anchor image media codesample table | hr removeformat | subscript superscript charmap | nonbreaking | visualchars visualblocks print preview fullscreen code";
        ...
        if(!defined('TINY_MCE_INIT_ONCE')) {
            ...
            $tpl .= file_get_contents("{$mce_path}tpl/tinymce_init_once.tpl");
        }
